...
const ProductCard = ({ item, visible, onClose }) => {
  ...
  const imageUri = Array.isArray(item.images) && item.images?.length>0 ?`https://api.shazlo.store/v1/items/getimage?url=${encodeURIComponent(
    item.images[cardimageindex]
  )}`:`https://api.shazlo.store/v1/items/getimage?url=${encodeURIComponent(
    item.image_url
  )}`;
