...
export default function SwipeUI({ brand, closet, closets, setclosets, setClosets }) {
  ...
  async function flushSeenBuffer() {
    try {
      ...
      await fetch("https://api.shazlo.store/v1/user/mark_seen_bulk", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const getitems = async (recommend, min_price, max_price, brands, products) => {
    ...
    try {
      ...
      const response = await fetch(
        'https://api.shazlo.store/v1/items/discover_similar',
        {
          method: 'POST',
          headers: {
  ...
  useEffect(() => {
    ...
    if (currentIndex + 2 < items.length) {
      ...
      const fullUri = `https://api.shazlo.store/v1/items/getimage?url=${encodeURIComponent(nextNextImageUrl)}`;
