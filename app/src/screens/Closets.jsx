...
const AnimatedClosetCard = React.forwardRef(({ item, onPressIn, onPressOut, onPress }, ref) => {
  ...
  return (
    ...
    >
      ...
      >
        {item.items.length > 0 && (
          <Image source={{ uri: `https://shaz-dmfl.onrender.com/v1/items/getimage?url=${encodeURIComponent(item?.items[0]?.image_url)}` }} style={styles.image} />
        )}
...
const MoodBoardsScreen = ({ setclosets, handleScreenChange}) => {
  ...
  useEffect(()=>{
    const getclosets=async()=>{
      setLoading(true);
      const response=await fetch(`https://shaz-dmfl.onrender.com/v1/closets/${user.user_id}`,{
        method:'GET'
      });
...
useEffect(() => {
  const handleDeepLink = async () => {
    if (route.name === 'Closet' && route.params?.id) {
      ...
      try {
        const response = await fetch(`https://shaz-dmfl.onrender.com/v1/closets/add-collab`, {
          method: 'POST',
