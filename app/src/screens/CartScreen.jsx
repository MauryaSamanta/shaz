...
// }, [data]);
  const getCart = async () => {
    const response = await fetch(`https://api.shazlo.store/v1/cart/${user.user_id}/`);
  ...
  const getAddresses = async () => {
    const response = await fetch(`https://api.shazlo.store/v1/address/${user.user_id}/`);
    const returnedData = await response.json();
  ...
  const removeItem = async (item_id) => {
    ...
    const response=await fetch('https://api.shazlo.store/v1/cart/remove/',{
      method:'POST',
  ...
  const handleCheckout=async()=>{ 
    ...
    if(step==="cart")
      ...
      if (addingnewadd) {
        ...
        const addressRes = await fetch('https://api.shazlo.store/v1/address/', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
    ...
    const orderRes = await fetch('https://api.shazlo.store/v1/order', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const renderItem = ({ item }) => (
    <View style={styles.card} >
      ...
      <Image source={{ uri: `https://api.shazlo.store/v1/items/getimage?url=${encodeURIComponent(item.image_url)}` }} style={styles.image} 
      />
      </TouchableWithoutFeedback>
