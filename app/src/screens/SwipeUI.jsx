...
translateY.setValue(0);
  ...
  items.forEach(item => {
    if (Array.isArray(item.images) && item.images.length > 0) {
      ...
      item.images.forEach(img => {
        const url = `https://shaz-dmfl.onrender.com/v1/items/getimage?url=${encodeURIComponent(img)}`;
        Image.prefetch(url);
      });
    } else if (item.image_url) {
      // Prefetch fallback main image
      const url = `https://shaz-dmfl.onrender.com/v1/items/getimage?url=${encodeURIComponent(item.image_url)}`;
      Image.prefetch(url);
    }
  });
};
  ...
  try {
    ...
    const response = await fetch(
      'https://shaz-dmfl.onrender.com/v1/items/getinitial',
      {
        method: 'POST',
        headers: {
