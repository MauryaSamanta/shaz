...
const PaymentScreen = ({ route, navigation }) => {
  ...
  const handleWebViewMessage = async (event) => {
    ...
    try {
      ...
      console.log(data)
      const response = await fetch('https://shaz-dmfl.onrender.com/v1/verify-payment/', {
  method: 'POST',
  headers: {
...
console.log(res)
      ...
      // Create order in DB
      const orderDbRes = await fetch('https://shaz-dmfl.onrender.com/v1/order-db/', {
        method: 'POST',
  ...
  const checkoutUrl = `https://shaz-dmfl.onrender.com/v1/razorpay-checkout/?key=${razorpay_key_id}&amount=${amount}&order_id=${order_id}&name=${user?.name}&mno=${user?.phone_number}`;
